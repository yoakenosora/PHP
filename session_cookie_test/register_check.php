...
?>
		<form action = "register.php" method = "post" name = "postForm">
			<input type="hidden" name="username" value="<?php $security->h($username);?>">
			<input type="hidden" name="csrf_token" value="<?php $security->h($_SESSION['csrf_token']);?>">
		</form>
		
<?php
		// リダイレクト
		$pageMove->redirect_to_register();		
